\section{Results and Analysis}\label{results-and-analysis}\label{sec:results}

\subsection{Main Comparison}\label{main-comparison}

Table~\ref{tab:main_results} answers the central question. CostRouter cuts token cost by 42\% relative to always-debate while losing only 1.5 percentage points of accuracy.

\begin{table}[h]
\centering
\caption{Routing strategy comparison across 82 tasks (GAIA, MATH, HumanEval, WebArena). Accuracy is strict full-credit. All topologies use Claude Opus 4.6 backbone. Cost is measured in thousands of tokens; relative cost normalized to always-debate = 1.00.}
...
\subsection{Routing Distribution and Per-Category Breakdown}\label{routing-distribution-and-per-category-breakdown}
...
\begin{table}[h]
\centering
\caption{Cost breakdown by task category. Token counts in thousands. CostRouter routes tasks within each category to the cheapest topology meeting $\tau = 0.6$.}
...
\subsection{Sensitivity to $\tau$}\label{sensitivity-to-tau}
