\section{CostRouter: Topology-Level Cost Routing}\label{costrouter-topology-level-cost-routing}
...
\subsection{Topology Cost Algebra}\label{topology-cost-algebra}
...
\textbf{Flat topology.} A single agent executes the task in a ReAct-style loop. One context window, one stream of generation. The token cost is simply the base cost of processing the query and generating a response:
...
\textbf{Hierarchical topology.} A coordinator agent receives the task, decomposes it into subtasks, and dispatches each to a specialist. Each specialist processes its subtask independently. At decomposition depth $D = 1$, the coordinator pays $c_{\text{base}}$ for planning and each of $N$ specialists pays $c_{\text{base}}$ for execution. But real hierarchical pipelines go deeper: a specialist may further decompose its subtask, creating $D$ levels of fan-out. At each level, the coordinator's context grows because it must track all outstanding subtasks. Summing across levels:
...
\subsection{Difficulty Estimation}\label{difficulty-estimation}
...
\tightlist
\item
  \textbf{Lexical complexity:} sentence count, average sentence length, vocabulary richness (type-token ratio)
\item
  \textbf{Structural indicators:} count of explicit subtask markers (``first,'' ``then,'' ``finally''), presence of conditional logic (``if,'' ``unless''), enumerated constraints
\item
  \textbf{Domain signals:} tool keywords (code, search, calculate, navigate), domain vocabulary density
\end{itemize}
...
\subsection{CostRouter Architecture}\label{costrouter-architecture}
...
\textbf{Algorithm 1: CostRouter Routing}
...
\subsection{Quality Threshold Selection}\label{quality-threshold-selection}
...
\subsection{Theoretical Cost Savings Bound}\label{theoretical-cost-savings-bound}
