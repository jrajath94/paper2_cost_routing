...
\subsection{Task Suite}\label{task-suite}
...
\subsection{Topology Candidates}\label{topology-candidates}
...
\tightlist
\item
  \textbf{Flat:} single ReAct-style agent in a tool-use loop. Cost: $c_{\text{base}}$.
\item
  \textbf{Hierarchical:} planner-worker pipeline with $N = 3$ specialist agents and decomposition depth $D = 2$. Cost: $6 \cdot c_{\text{base}}$.
\item
  \textbf{Debate:} two proposer agents plus a judge, running $R = 4$ rounds. Cost: $\sim 24\text{-}36 \cdot c_{\text{base}}$ depending on judge verbosity.
\end{itemize}
...
\tightlist
\item
  \textbf{Always-flat:} route every query to flat. Cheapest possible, but accuracy-limited on hard tasks.
\item
  \textbf{Always-hierarchical:} route everything to hierarchical. Moderate cost, strong on decomposable tasks.
\item
  \textbf{Always-debate:} route everything to debate. Most expensive, strongest on iterative reasoning.
\item
  \textbf{FrugalGPT-adapted:} we adapt the FrugalGPT cascade \cite{chen_2023} from model routing to topology routing. Queries start flat; if a lightweight confidence check flags low confidence, the query escalates to hierarchical, then debate. This sequential cascade adds latency but provides a natural comparison point.
\item
  \textbf{BAMAS-adapted:} we adapt Budget-Aware Agentic Routing \cite{zhang_2026} from model selection to topology selection, using its boundary-guided training objective to learn a routing policy under a fixed total budget constraint.
\item
  \textbf{Oracle:} selects the cheapest topology that actually succeeds on each query, computed with perfect hindsight from the full results matrix. No real system can match this.
\end{itemize}
...
\tightlist
\item
  \textbf{Total token cost:} sum of tokens consumed across all test queries. The primary cost metric.
\item
  \textbf{Accuracy:} fraction of queries answered correctly. The primary quality metric.
\item
  \textbf{Cost-per-correct:} total token cost divided by correct answers. This penalizes methods that spend tokens on queries they ultimately get wrong.
\item
  \textbf{Pareto efficiency:} area under the cost-accuracy Pareto curve when sweeping $\tau$. Higher means better cost-quality tradeoff across the full range of operating points.
\end{itemize}
